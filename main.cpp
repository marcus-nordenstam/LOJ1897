...
HWND CreateSplashScreen(HINSTANCE hInstance) {
    ...
    HWND hSplash = CreateWindowExW(
            ...
            NULL,
            WS_POPUP,
            (GetSystemMetrics(SM_CXSCREEN) - splashWidth) / 2,
...
void LoadIconsFromPng(HINSTANCE hInstance, const std::wstring &pngPath, HICON &hIconLarge, HICON &hIconSmall) {
    hIconLarge = NULL;
    hIconSmall = NULL;
...
int APIENTRY wWinMain(_In_ HINSTANCE hInstance,
    ...
    static auto WndProc = [](HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) -> LRESULT {
        switch (message) {
            ...
            case WM_DPICHANGED:
                ...
                break;
            case WM_CHAR:
                // Forward to Noesis first if available
            ...
            case WM_LBUTTONDOWN:
                if (g_noesisRenderPath && g_noesisRenderPath->GetNoesisView()) {
                    int x = LOWORD(lParam);
                    int y = HIWORD(lParam);
                    g_noesisRenderPath->GetNoesisView()->MouseButtonDown(x, y, Noesis::MouseButton_Left);
                }
                break;
            case WM_LBUTTONUP:
                if (g_noesisRenderPath && g_noesisRenderPath->GetNoesisView()) {
            ...
            case WM_RBUTTONDOWN:
                if (g_noesisRenderPath && g_noesisRenderPath->GetNoesisView()) {
                    int x = LOWORD(lParam);
                    int y = HIWORD(lParam);
                    g_noesisRenderPath->GetNoesisView()->MouseButtonDown(x, y, Noesis::MouseButton_Right);
                }
                break;
            case WM_RBUTTONUP:
                if (g_noesisRenderPath && g_noesisRenderPath->GetNoesisView()) {
    ...
    HICON hIconSm = (HICON) LoadImage(hInstance, MAKEINTRESOURCE(IDI_MAIN_ICON), IMAGE_ICON, 16, 16, LR_DEFAULTCOLOR);
